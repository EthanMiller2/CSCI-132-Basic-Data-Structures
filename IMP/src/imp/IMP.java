...
class IMP implements MouseListener {
   ...
   //Instance Fields you will be using below
    ...
    private void getValue() {
        ...
        color = getPixelArray(pix);
        System.out.println("Color value " + color[0] + " " + color[1] + " " + color[2] + " " + color[3]);
    }
   ...
   ************************************************************************************************
    ...
    private void fun2() {
        ...
        s.push(new PixelRegion(colorX, colorY));
        ...
        rgbArray[2] = 0;
        rgbArray[3] = 0;
        picture[colorX][colorY] = getPixels(rgbArray);

        while (!s.empty()) {
            int a = s.peek().getCount();
            int h = s.peek().getH();
            int w = s.peek().getW();
            ...
            switch (a) {
                case 0:
                    rgb = getPixelArray(picture[h - 1][w - 1]);
            
                    if (rgb[3] == rgbArray[3]) {
                        picture[h - 1][w - 1] = getPixels(rgbArray);
                        s.push(new PixelRegion(h - 1, w - 1));
                    }
                    break;

                case 1:
                    rgb = getPixelArray(picture[h - 1][w]);
                
                    if (rgb[3] == color[3]) {
                        picture[h - 1][w] = getPixels(rgbArray);
                        s.push(new PixelRegion(h - 1, w));
                    }
                    break;
                ...
                case 3:
                    rgb = getPixelArray(picture[h][w - 1]);
                  

                    if (rgb[3] == color[3]) {
                        picture[h][w - 1] = getPixels(rgbArray);
                        s.push(new PixelRegion(h, w - 1));
                    }
                    break;
